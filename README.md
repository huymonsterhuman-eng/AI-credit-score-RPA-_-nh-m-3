# AI Credit Classification kết hợp RPA trong quy trình xét duyệt tín dụng

> Đồ án môn **Công nghệ dịch vụ tài chính** — Nhóm 3
> Hệ thống **phân loại mức tín dụng** (Poor / Standard / Good) dựa trên Machine Learning kết hợp tự động hóa quy trình (RPA-style) qua n8n để ghi log Google Sheets, gửi email báo cáo kết quả cho khách hàng.
>
> *Ghi chú: tên đề tài chính thức đang chờ nhóm chốt lại — hiện tại đề xuất "AI Credit Classification + RPA".*
...
- Train mô hình ML **phân loại mức tín dụng** (Poor / Standard / Good) từ thông tin cá nhân/tài chính khách hàng.
- Cung cấp giao diện web (Streamlit) cho phép nhập thông tin và nhận kết quả kèm giải thích (SHAP).
- Tự động hóa hậu xử lý qua n8n: lưu Google Sheets → gửi email báo cáo theo mức tín dụng.
...
│                                                                │
│   Form nhập liệu → Predict → Nhóm phân loại + Gợi ý            │
│   ├─ Preset buttons (Poor/Standard/Good)                       │
│   ├─ Xác suất theo từng nhóm                                   │
│   └─ SHAP explanation                                          │
└──────────────────────────────┬─────────────────────────────────┘
                               │  POST webhook
                               ▼
...
## 5. Bảng phân loại tín dụng

| Nhóm | Mô tả | Ý nghĩa |
|---|---|---|
| **Poor** (Kém) | Rủi ro cao | Hầu hết ngân hàng từ chối, hoặc yêu cầu lãi suất cao |
| **Standard** (Trung bình) | Rủi ro vừa | Có thể được duyệt vay, lãi suất chưa tối ưu |
| **Good** (Tốt) | Rủi ro thấp | Đủ điều kiện vay với lãi suất ưu đãi |
